...
function normalizeStatus(value: string | undefined): ListingStatusFilter {
  return value === "active" || value === "draft" || value === "inactive" ? value : "all";
}
        ...
        <form action="/dashboard/listings" method="get" className="grid gap-3 p-4 lg:grid-cols-[minmax(180px,1fr)_150px_150px_190px_180px_auto] lg:items-end">
          <label className="grid gap-1.5">
            ...
            <input
              ...
              className="h-10 rounded-[8px] border border-[var(--seller-rule-strong)] bg-[var(--seller-surface)] px-3 text-[13px] outline-none focus:border-[var(--seller-brand)] focus:ring-2 focus:ring-[var(--seller-accent-soft)]"
            />
          </label>

          <label className="grid gap-1.5">
            <span className="text-[11px] font-semibold uppercase text-[var(--seller-muted)]">Status</span>
            <select name="status" defaultValue={status} className="h-10 rounded-[8px] border border-[var(--seller-rule-strong)] bg-[var(--seller-surface)] px-3 text-[12px] outline-none focus:border-[var(--seller-brand)] focus:ring-2 focus:ring-[var(--seller-accent-soft)]">
          ...
          <label className="grid gap-1.5">
            ...
            <select name="mode" defaultValue={mode} className="h-10 rounded-[8px] border border-[var(--seller-rule-strong)] bg-[var(--seller-surface)] px-3 text-[12px] outline-none focus:border-[var(--seller-brand)] focus:ring-2 focus:ring-[var(--seller-accent-soft)]">
          ...
          <label className="grid gap-1.5">
            ...
            <select name="issue" defaultValue={issue} className="h-10 rounded-[8px] border border-[var(--seller-rule-strong)] bg-[var(--seller-surface)] px-3 text-[12px] outline-none focus:border-[var(--seller-brand)] focus:ring-2 focus:ring-[var(--seller-accent-soft)]">
          ...
          <label className="grid gap-1.5">
            ...
            <select name="sort" defaultValue={sort} className="h-10 rounded-[8px] border border-[var(--seller-rule-strong)] bg-[var(--seller-surface)] px-3 text-[12px] outline-none focus:border-[var(--seller-brand)] focus:ring-2 focus:ring-[var(--seller-accent-soft)]">
          ...
          <button type="submit" className="h-10 rounded-[8px] bg-[var(--seller-brand)] px-4 text-[12px] font-semibold text-white transition hover:brightness-95">
            Terapkan
          </button>
        </form>
      ...
      <Panel>
        {sellerListings.length === 0 ? (
          ...
          <EmptyState
            ...
            action={<Link href={buildListingsHref(current, { q: null, status: "all", mode: "all", issue: "all", sort: "updated-desc" })} className="inline-flex h-10 items-center rounded-[8px] border border-[var(--seller-rule-strong)] px-4 text-[12px] font-semibold text-[var(--seller-brand)] hover:bg-[var(--seller-brand-soft)]">Reset filter</Link>}
          />
        ) : (
